"""Context-sensitive severity adjustment based on file path.

Test files get reduced severity (a pickle.loads in a test is low risk).
API handlers keep or increase severity. Migrations get reduced to low.
"""
...
def adjust_severity(base_severity: str, file_path: str) -> str:
    """Adjust finding severity based on file path context.
    ...
    for pat in _REDUCE_PATTERNS:
        if pat.search(file_path):
            return _shift_severity(base_severity, -1)
